...
parser.add_argument('--patience', help="Number of epochs to wait before early stop if no progress on the validation set. Default = 30, type=int")
parser.add_argument('--min_delta', help="Change in loss function to be considered as improvement for EarlyStopping (float). Default = 0.0001, type=float")
...
MIN_DELTA = float(sys.argv[18])
METADATA = sys.argv[20]

with open(sys.argv[20], 'r', newline='') as f:
...
plt_a.set_xlabel('Lesion Class', fontsize=14)
plt_a.set_title('Lesion Class', fontsize=15)
...
plt_c.set_xlabel('Body Part', fontsize=14)
plt_c.set_title('Lesion coounts by body part', fontsize=15)
...
a.axes.set_title("Distribution of lesions by age",fontsize=15)
...
keras_callbacks = [
      EarlyStopping(monitor='val_loss', patience=PATIENCE, mode='min', min_delta=MIN_DELTA),  # comment this line to deactivate EarlyStopping
...
print(bcolors.OKRED + "--- Cleaning up base_dir and weights ---" + bcolors.ENDC)
print("")
