...
print("")
lesion_counts = metadata['dx'].value_counts() # counting occurrences by class
...
sex_counts = metadata['sex'].value_counts() # counting occurrences by class
...
localization_counts = metadata['localization'].value_counts() # counting occurrences by class
...
expert_validation_counts = metadata['dx_type'].value_counts()      # counting occurrences by class
...
print(bcolors.OKGREEN + "--- Resizing images and performing data augmentation ---" + bcolors.ENDC)
print("")
...
X = X/255.  # Scale values ​​to 0-1. You can also use StandardScaler or other method
Y=skin_df_balanced['label']  # Assigning label values ​​to Y
Y_cat = to_categorical(Y, num_classes=7) # Converting variables to categorical variables, since this is a classification problem
print("")
...
model = Sequential([data_augmentation,
	layers.Conv2D(256, (3, 3), activation="relu", input_shape=(SIZE, SIZE, 3)),
...
#model.summary()    # preguntar a cristobal

model.compile(loss='categorical_crossentropy', optimizer='Adam', metrics=['acc'])

print(bcolors.OKCYAN + "--- Training. You can also use generator to use augmentation during training ---" + bcolors.ENDC)
print("")
print(bcolors.OKCYAN + "--- We will also employ Early Stopping and training models will be saved with a certain frequency---" + bcolors.ENDC)
print("")
